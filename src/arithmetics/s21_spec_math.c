...
void add_mantissas(s21_big_decimal val1, s21_big_decimal val2,
                   s21_big_decimal *res) {
...
void subtract_mantissas(s21_big_decimal val1, s21_big_decimal val2,
                        s21_big_decimal *res) {
  long long borrow = 0;
...
__uint64_t s21_muldiv_int(s21_big_decimal *big_dcml, int operation, int operand,
                          int iterations) {
...
__uint64_t div_int(s21_decimal *value, int operand) {
  __uint64_t rem = 0, tmp = 0;
  for (int i = 2; i >= 0; i--) {
    tmp = (rem << 32) | value->bits[i];
    rem = tmp % operand;
    value->bits[i] = tmp / operand;
  }
  return rem;
}

void multiply_to_exp(s21_decimal *dst, int *exp) {
  s21_big_decimal temp = s21_conversion_from_dec_to_ldec(*dst);
  s21_muldiv_int(&temp, mul, 10, -(*exp));
  *exp = 0;
  big_to_dcml(temp, dst);
}
